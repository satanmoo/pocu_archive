...
함수 reciever 원본을 바꾸고 싶으면 ref 키워드 사용
...
참고) 지역 변수 선언 시 ref 키워드를 사용할 때는 양변에 모두 ref 키워드가 있어야함

https://learn.microsoft.com/ko-kr/dotnet/csharp/language-reference/statements/declarations#reference-variables
...
CPU에서 참조형을 읽기 위해 메모리의 데이터를 읽어오고, 참조형 변수를 수정하면 메모리의 원본 데이터가 수정됨

## 참조형과 쓰레기 수집기

![alt text](image-4.png)

참조형은 GC의 대상이기 때문에 참조형 임시 데이터를 너무 많이 만들면 GC의 성능저하가 올 수 있다.

값형은 GC의 대상이 아니다. 하지만 복사에 따른 비용에 따라 성능저하가 올 수 있다.
...
참고로 SwapHuman의 매개변수에서 ref 키워드를 제거하면 Swap은 정상 작동하지 않는다.

대신 메서드 내 지역 참조 변수의 값을 스왑한다.

함수 스코프 안에서 human1이 human2를 참조하고, human2는 human1을 참조하고 스코프가 끝나면 이 지역변수가 사라져 원본은 그대로이다.

```C#
public static void SwapHuman(Human human1, Human human2)
{
     Human temp = human2;
     human2 = human1;
     human1 = temp;
...
> Struct with field initializers must include an explicitly declared constructor
...
대신 지역변수 구조체 v의 값만 바뀜. 여기에 접근할 수 있는 방법은 없다.
...
개체를 생성할 때 새로운 방법이다. 배열 생성과 초기화처럼 클래스나 구조체도 개체를 생성하면서 멤버 변수를 초기화할 수 있다.
...
- Head가 구조체인 경우 구조체를 만들고 구조체 내부 값은 0에 준하는 값으로 초기화 된다.
...
값형은 cpu에 있는데 null은 메모리에 없다는 개념이다. 각각 개념이 다르다.
> 값이 0이라는 것은 CPU의 저장된 변수 값이 0이라는 것
> 참조형이 null이라는 것은 메모리에 없다는 것
