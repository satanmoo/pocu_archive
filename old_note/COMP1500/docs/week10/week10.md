...
문자열을 더할 때도 왼쪽 연산자가 오른쪽 연산자보다 우선적으로 평가된다.
...
여기서 생각해볼 점은 `join도 성능 면에서 효율적이지 않다`는 것이다. join은 내부에서 위의 코드와 마찬가지로 반복문을 통해 문자를 더하기 때문에 버려지는 임시 문자열을 많이 생성하게 된다.
...
주목할 점은 ensure이라는 동사다. 1024의 공간을 확보한 상태에서 512로 공간을 줄일 수 있을까? 불가능하다. EnsureCapacity함수는 공간을 `늘리는 기능`만 제공한다. 공간의 용량을 마음대로 변화시키는 것은 아니다.  
...
start 번째임을 주의하자.
count는 색인 개수를 세는 의미를 가진다. start(포함)부터 start + count(미포함)까지 범위를 지정
...
>> 부동소수점 <-> decimal일 때는 반드시 명시적 형변환 해줘야함
