...
- 메인 함수가 외부로 받는 데이터
    ...
    - 메서드 인자라고도 부른다.
- 메인 함수의 인자로 데이터를 받는 방법은 커맨드 라인으로 부터 받는다. 이 때 커맨드 라인 인자가 string[] args로 저장되어 메인 함수에서 사용된다.
...
- 메인 함수의 경우 정수 값을 반환하게 할 수 있다. 이럴 때는 반환형으로 `int`를 사용하고 `return`이라는 키워드를 사용한다.
    ...
    - `커맨드 라인`에서 Main 함수의 반환형을 받아서 확인할 수 있다.
        - 0 : 대부분의 경우 성공을 의미한다.
        - 0이 아닌값 : 오류 코드
            - 이 오류 코드는 프로그램마다 굉장히 다양하고, 보통 사용자에게 메뉴얼로 오류 코드가 무엇을 의미하는지 알려준다.
...
절차적 프로그래밍에서 상태를 저장하는 곳을 따로 둔다고 했는데 그것이 변수다.
...
- 하나의 문자를 담는 자료형
    - 알파벳, 숫자, 특수문자 등등을 화면에 출력할 때 보여주기 위함
    - 문자를 감쌀 때 작은 따옴표 사용
...
- 사람들끼리 각 문자를 숫자와 1대1 대응하기러 약속했습니다. 그 약속이 [ASCII](https://ko.wikipedia.org/wiki/ASCII)죠?
    ...
    - 128부터 2진수로 표현하면 가장 왼쪽 부호 비트자리에 숫자 1을 넣어야합니다. 따라서 문자열을 음수로 표현해야하는 문제(?)가 생겼습니다. 사람들은 문자를 음수에 대응하는 것을 동의하지 않았고 모두
      양수에 대응했습니다.
