...
- 메모리 주소도 숫자니까 변수에 저장할 수 있지 않을까?(4바이트 짜리 숫자?)
...
- 하드웨어는 자료형을 신경쓰지 않음
- 비트패턴은 그대로, 하지만 해석이 바뀐다.
  - int 형으로 읽으면 32비트 읽죠.
  - char 형으로 읽으면 8비트를 읽죠.

### 프로그래밍 언어 입장에서 자료형 개념이 존재한다.

![img_14.png](img_14.png)

- 따라서 하드웨어에 해당 주소로 부터 몇 바이트 읽어야하는지 알려줘야한다.
- 이것이 포인터 변수의 자료형이다.
...
- 지역 변수가 아닌 경우: 함수 스택 프레임 안에 속하지 않는 경우
  - 전역 변수
  - 함수 내부의 static 변수
  - 파일 속 static 전역 변수(extern 해서 못 쓰게 막음, 링킹 오류)
...
- 어떤 함수에 포인터가 들어올 때 그 포인터가 유효하다고 믿고 싶다.
- 함수의 선조건 문제
...
- free를 호출해도 ptr이라는 포인터에 값이 지워지지는 않는다. 즉 더 이상 사용하지 않는 주소가 남아있음
...
- 그리고 이 주소값을 int_ptr에 대입했. 따라서 41/00/00/00으로 읽죠? 엄청 큰 값이 나옴!
...
- 함수 안에서 사용하면 문자열은 함수 스택 프레임이 생길 때, 데이터 섹션으로 부터 문자를 복사해서 생성된다.
- 함수 밖이라면 데이터 섹션에 들어가있다.
- 포인터 변수로 문자열을 초기화하면 포인터 변수는 스택에 저장된다.
- 문자열은 데이터 섹션에 저장된다. 단지 데이터 섹션의 주소값이 포인터 변수에 저장된다.
- 스택에 저장된 문자열은 수정해도 괜찮다. 데이터 섹션에 저장된 문자열은 수정하면 안된다.
  - 결과가 정의되지 않음. 즉 문자열이 읽기 전용임
...
- do_magic의 매개변수에 필요한 것은 int*의 배열이다. 근데 2차원 배열은 그냥 int배열이다. 주소값의 배열이 아니다. 헷갈리지 말죠!
