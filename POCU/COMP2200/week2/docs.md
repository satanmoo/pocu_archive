...
- ESP에 EBP 대입
- 0x0073FE6C에 main함수의 호출이 끝나고 돌아갈 주소가 있다.
    - 함수 스택프레임의 시작에는 호출자의 주소가 있다!
...
- 14h(20byte) 만큼 내려감
    - 그림에 한 칸에 4바이트임
- main 함수의 스택 프레임 결정
    - esp는 ebp랑 동일한 값이었는데, esp는 ebp - 14h가 되었다.
...
- mov eax,dword ptr [ebp-0Ch]
    - eax는 레지스터, CPU에서 계산을 위해 임시로 값을 저장하는 공
...
- add함수의 제일 처음 코드
- add함수 끝나고 돌아올 main함수의 주소를 스택 메모리에 push한다.
    - ebp에 main함수의 시작 주소가 저장되어 있다.(0x0073FE6C)
- 여기서 원리는 어떤 함수의 스택 프레임 시작에는 호출자의 주소, 즉 돌아갈 주소를 저장한다는 것
...
- pop을 실행하면 ebp에 esp주소에 있는 값을 대입한다. 그리고 esp는 한 칸 줄어든다.
- esp의 값(0x0073FE6C, 메인 함수 스택 프레임 시작 주소)를 ebp에 넣음, esp는 증가했다.
    - pop 연산은 스택 메모리에서 가장 마지막(esp에 저장된 값)을 빼서, 인자(여기서는 ebp)에 넣어주는 연산이다.

#### [00011064] ret

![img_181.png](img_181.png)

- return esp에 저장된 주소(0001102D)로 점프해서 돌아가라
- esp를 pop하고 그 주소로 돌아가, 따라서 esp는 한 칸 위로 올라가게 된다.
...
- [0001102D] add esp,14h를 실행하면?
    - esp에 14h(20byte) 더함
    - 즉 메인 함수 스택 프레임 없어지는 과정

- [00011030] pop ebp
    - esp에 저장된 주소(메인함수 호출자의 주소)를 ebp에 넣음
    - 스택 프레임이 줄어듬
