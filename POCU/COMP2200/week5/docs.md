...
- 깊은 복사 vs 얉은 복사
  - C/C++ 업계에서 흔히 따르는 원칙은 외부에서 전달된 포인터를 내부에서 저장하지 않는다.
  - 저장할 때는 깊은 복사

- scanf에서 stdin의 공백을 무시하고 읽는다.
  - stdin에서 읽을 위치를 가리키는 포인터가 공백을 건너뛴다.
  - stdin은 스트림이라 한번 읽을때 마다 다음에 읽을 위치를 가리키는 포인터가 이동함