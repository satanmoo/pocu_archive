...
- *pp == NULL이면 빈 연결 리스트
- 바로 새로 만든 노드의 next 값을 추가하는 코드로 넘어감
- 새로운 노드의 next에 NULL 대입

![img_93.png](img_93.png)

- pp는 연결 리스트의 헤드(즉 첫번째 노드의 주소값을 가지는 변수)의 주소값을 가짐
- 새로운 노드가 첫번째 노드라 *pp에 첫번째 노드의 주소값을 대입

[새로운 노드를 끝에 추가하기]

![img_94.png](img_94.png)

- 값을 비교하고, 새로 추가할 값보다 작으니까 break로 빠져나오지 않음
- *pp: 첫번째 노드의 주소값을 가짐
  - 값은 2가 들어있는 노드의 주소
- *pp->next: pp가 가리키는 노드의 다음 노드의 주소값을 가짐
  - 연결 리스트에 노드가 하나밖에 없으니, 값은 NULL
- pp = &((*pp)->next): NULL(값0)을 가지는 메모리 주소를 저장함
...
- pp: 2값을 가지는 노드의 next의 주소를 가짐
- *pp: 2값을 가지는 노드의 next의 주소값에서 역참조하니까 2값을 가지는 노드의 next == 5값을 가지는 노드의 주소
- *pp->value: 5
...
- 2값을 가지는 노드의 next에 새로 추가할 노드(3을 가진 노드)의 주소값 대입
