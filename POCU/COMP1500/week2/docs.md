...
- 실행파일(exe 파일)을 실행하면 자동으로 메인 함수가 실행된다.
...
Hi, C#, is fun 총 4개가 인자로 들어간다. 따라서 args가 복수형이죠?
...
- 메인 함수의 경우 정수 값을 반환하게 할 수 있다. 이럴 때는 반환형으로 `int`를 사용하고 `return`이라는 키워드를 사용한다.
  - 메인 함수에서 반환형의 역할은 exe 프로그램이 올바르게 실행됬는지 여부를 알려준다. `커맨드 라인`에서 Main 함수의 반환형을 받아서 확인할 수 있다.
    - 0 : 대부분의 경우 성공을 의미한다.
    - 0이 아닌값 : 오류 코드
