...
![인터프리터](../image/week1/image-2.png)

- 소스코드를 인터프리터가 `실행 중 기계어로 변환`한다. 근데 이 변환도 애매한 표현이다. 왜? 컴파일러가 기계어로 하나하나 번역하는 방식과 다르게, 어떤 `기계어들로 이루어진 집합`(대표적으로 함수가 있다.)으로 번역하게 된다. 그리고 이 기계어들로 이루어진 집합이 실행된다.
- Javsscript, Python, PHP 등이 있다.
- 실제로 실행하기 전까지 코드의 문제를 잡아낼 수 없다는 단점이 있다.

## 강한 타입(Strong-typed) vs 약한 타입(Weak-typed)

변수를 선언한 후에 변수형을 변경할 수 있는지 여부에 따라 분류한다.
...
이 개념은 컴파일 언어의 장점과 관련이 있다. 컴파일 언어가 인터프리터 언어보다 좋은 이유도 상용 코드를 작성할 때 컴파일러가 실수를 잡아줄 수 있다는 장점 때문이다.

## 매니지드 언어 vs 언매니지드 언어

누가 메모리의 life cycle(수명)을 관리하냐에 따라 분류할 수 있다. 메모리의 수명을 관리하는 것이 왜 필요할까? 컴퓨터의 구조 상 프로그램이 실행되려먼 운영체제로 부터 `독점적인` 메모리를 받아서 `확보`해야한다. 그리고 프로그램의 실행이 끝나면 받은 메모리를 운영체제에 `반납`해야한다.

언매니지드 언어는 프로그래머가 직접 메모리 반납을 수행한다. 반면 매니지드 언어는 언어 자체적인 기능으로 더 이상 프로그램이 메모리를 사용하지 않는다고 판단해 메모리를 반환해준다.

따라서 매니지드 언어가 실수가 적고 편하다. 그러나 효율성이 떨어질 수 있다. 언어 자체 기능이 사람보다 명확하지 않을 수 있다!
...
설명하기 굉장히 어려운 개념인데 실생활 예제는 장부에 하루 결산 금액을 저장하고 오늘 영업 시간에 수입과 지출을 더하고 빼면서 계산을 한다. 즉 계산을 하는 기능과 장부라는 저장이 분리된 개념이다. 여기서 중요한 것은 계산 과정에서 수입, 지출 등 항목은 지속적으로 변할 수 있다는 것이다. 상태가 변할 때 마다 덮어쓰기 때문이다. 물론 최종 계산 결과는 모두 누적되서 장부에 저장된다. 즉 절차적 언어에서는 기능과 자료 저장이 분리되어있으며, 기능은 상태 변화를 동반할 수 있다.

## 프로그래밍 패러다임 : OOP 언어

- 객체지향(Object Oriented Programming)
- 사람에게 가장 직관적인 패러다임 따라서 널리 쓰인다.
- 기능과 자료를 하나의 개체(Object)에 합친다.

## 프로그래밍 패러다임 : 함수형 언어

- 절차적 언어처럼 함수가 있지만 상태의 변화가 없다.
- 불변성(immutable)

절차적 언어는 상태를 함수(기능)으로 계속 변화시켰다. 근데 함수형 언어는 아니다..?  
실생활 예제를 살펴보자. 절차형 언어의 예제에서 수입과 지출을 기록할 때 덮어쓰지 않는다. 어떤 거래가 발생할 때 마다 누적적으로 수입과 지출을 모두 기록한다. 즉 수입과 지출이라는 항목의 상태가 변하는 것이 아니다. 이렇게 상태가 변하지 않는 것이 왜 중요할까? 멀티 스레딩에서 동기화 문제와 연결된다. `race condition`
