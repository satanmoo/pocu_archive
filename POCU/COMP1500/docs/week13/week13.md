...
readonly는 runtime 상수라 생각하면 편힘

### GetCurrentDirrectory()

현재 실행 컥텍스트의 경로(WORKING DIRECTORY)를 가져옴
...
FileAcceess ENUM은 읽기, 쓰기, 읽고 쓰기 권한을 나타냄
...
\* 이진 파일을 읽을 때는 바이너리를 읽어 각 자료형으로, 이진 파일로 쓸 때는 각 자료형을 바이너리로 쓴다. 결국 자료형도 이진 패턴을 목적에 따라 분류한 것임을 알 수 있다.
...
writer은 아무런 영향도 받지 않는다. writer만의 stream내의 위치를 기억하기 때문이다.(내부의 버퍼 때문이다.)

writer.Write("Overwritten text");의 결과 지금까지 write한 텍스트 뒤 즉 기존 스트림 뒤에 Overwritten text가 append된다.

그렇다면 만약에 overwrite 즉 덮어쓰고 싶으면 어떻게 해야할까?

BaseStream에서 Seek을 통해 스트림의 처음으로 돌아가고 다시 writer을 생성하면 된다. 아니면 Writer가 사용하는 버퍼를 비우는 flush를 해준 뒤 Seek으로 파일 스트림의 처음으로 돌아가 쓰면 된다.
