...
![Array.Copy](../image/week8/image-4.png)
...
![Alt text](../image/week8/image-5.png)
...
![Alt text](../image/week8/image-6.png)

짜잔! 이것이 Json이다. 예전에는 XML도 많이 썼다.

### 표

![Alt text](../image/week8/image-7.png)
...
![Alt text](../image/week8/image-8.png)
...
![Alt text](../image/week8/image-9.png)
...
![Alt text](../image/week8/image-11.png)
...
```C#
string[] MyTokenizer(string origin, char[] delimiters)
{
    ...
    while (startIndex < origin.Length)
    {
        ...
        if (delimiterIndex == -1)
        {
            ...
            {
                res[cnt++] = origin.Substring(startIndex);
            }
            break;
        }
        ...
        if (startIndex != delimiterIndex) 
        {
			...
			{
				string[] newArray = new string[res.Length * 2];
				Array.Copy(res, newArray, cnt);
				res = newArray;
				res[cnt++] = token; 
			}
        }
    ...
    string[] finalTokens = new string[cnt];
    Array.Copy(res, finalTokens, cnt);
    return finalTokens;
}

int findDelimiterIndex(string input, char[] delimiters, int start)
{
	for (int i = start; i < input.Length; i++)
	{
		char inputCharacter = input[i];
		foreach (cahr delimiter in delimiters)
		{
...
![Alt text](../image/week8/image-12.png)
...
![Alt text](../image/week8/image-13.png)

- delimiters는 string []

## 구분 문자 사이가 비어있다면?

![Alt text](../image/week8/image-15.png)

![Alt text](../image/week8/image-14.png)

[열거형 : RemoveEmptyEntries](https://learn.microsoft.com/ko-kr/dotnet/api/system.stringsplitoptions?view=net-8.0)

## 불필요한 공백 지우기

![Alt text](../image/week8/image-16.png)
...
![Alt text](../image/week8/image-17.png)
...
![Alt text](../image/week8/image-18.png)
- 매개변수가 아예 승격이 불가능한 경우 : 오버로딩 OK
![Alt text](../image/week8/image-19.png)
...
```C#
static string getFullAddress(string street, string city, strint state = "");
```
...
![Alt text](../image/week8/image-20.png)

- 수정 된 함수를 호출하는 다른 사람이 의도하지 않게 호출하게 된다.

![Alt text](../image/week8/image-21.png)

- 2) 기본값 인자가 도중에 변경된 경우 기존의 코드에서 문제가 발생할 수 있다. 

![Alt text](../image/week8/image-22.png)
...
![Alt text](../image/week8/image-25.png)

### 키보드 입력

![Alt text](../image/week8/image-26.png)
