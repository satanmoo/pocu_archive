...
함수 시그니처는 아래와 같이 구성된다. 시그니처는 고유한 함수를 식별할 수 있게 해준다. 함수의 identity
...
- void 형은 반환값이 없다.
- void가 아닌 경우 return 키워드를 이용해 데이터를 반환해야 한다. 그렇지 않으면 `컴파일 오류`가 발생한다.
...
메인 함수는 언제 실행될까? 프로그램이 시작되면 바로 실행된다. 따라서 프로그램을 실행할 때 어떤 인자를 입력해야지 메인 함수는 인자를 받아서 출력을 하던 함수 바디의 내용을 실행한다!
...
- `함수의 블랙박스` : 어떤 입력을 넣으면 함수 내부 구조(작동 원리)를 알지 못한다.
...
- 선조건
  ...
  - 함수를 만드는 프로그래머의 문제인지, 사용하는 사람의 문제인지? Divide 함수의 분모에 0을 넣으면 사용하는 사람 문제겠죠?
...
- 결국 함수 구현부를 봐야한다.
- direction이 도대체 뭔데???
- 이걸 Enum으로 바꾸면 Enum을 확인하고 함수의 선조건을 알 수 있죠?
...
### ❗️C#에서 지원하는 다른 방법(질문 필요) 열거형3, 6분쯤
...
- 디버그 모드에서만 사용가능하다.
  ...
  - 릴리즈 모드에서 동작하면 성능 저하가 발생한다.

### Debug.Assert()

- using System.Diagnostics 라이브러리를 추가해야한다.
- Assert()안에 들어가는 조건이 거짓일 때 프로그램은 종료되고, 어서트 메세지를 출력한다.
